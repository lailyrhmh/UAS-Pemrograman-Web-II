...
@section('content')
    <div class="tabs-container">
        <ul class="nav nav-tabs">
            <li><a class="nav-link active" data-toggle="tab" href="#tab-1"> Trainee List</a></li>
        </ul>
        <div class="tab-content">
            <div id="tab-1" class="tab-pane active">
                <div class="panel-body">
                    <form action="{{ route('trainee.update', $trainees->id) }}" method="POST">
                        @csrf
                        <!-- {{ csrf_field() }} -->
                        @method('PUT')
                        ...
                        <div class="form-group row"><label class="col-sm-2 col-form-label">Trainee Name :</label>
                            <div class="col-sm-10"><input type="text" name="trainee_name" id="trainee_name" class="form-control"
                                    placeholder="Input Trainee Name" value="{{ old('trainee_name', $trainees->trainee_name) }}"></div>
                        </div>
                        <div class="form-group row"><label class="col-sm-2 col-form-label">Label ID :</label>
                            <select class="form-control"  name="label_id" id="label_id">
                                <option value="{{ $trainees->label_id }}">{{ $trainees->label->label_name }}</option>
